...
#### 三.配置nginx
安装nginx完成后，如果能正常打开`myblog.viking666.com`，说明下面环节是通畅的：
...
然后检查media文件是否已经正常拉起：
在目录`/www/myblog/media` directory下添加文件meida.png，然后访问`myblog.viking666.com:8000/media/media.png` ，成功后进行下一步测试。
...
```
访问myblog.viking666.com:8000 ,如果显示`hello world`，则下面环节是否通畅:
```
...
```
打开 `myblog.viking666.com:8000` ，看看是否成功
